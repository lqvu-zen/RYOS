...
**Test suite:** grew from a **red 45-passing baseline** (2 stale tests were failing) to **129 passing / 3 skipped**, now covering interpreter (`detect_interpreter`, `build_command`, `resolve_interpreter` incl. the RYOS.exe guard, `_script_tag`), DB CRUD/ordering/export/migrations + versioning + groups (rename propagation, orphan-on-delete, deep clone) + pipelines (steps join, cascade delete, deep clone, reorder) + `mark_run_status`, `settings` load/save fallbacks + round-trip, `notifications._parse_version`, the full `quickrun` module, and `jobs` (`Job` defaults, `format_elapsed`, `JobRegistry`).
