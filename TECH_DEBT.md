...
| 6 | `RYOSApp` god class — ~100 methods, 2,199 lines | Architecture | **16** | ✅ Done — job lifecycle (allocation, capacity, pipeline stepping, step completion, output drain) extracted into UI-free `job_controller.JobController` + tested; only widget construction (`_add_running_row`, `_finish_job` teardown) stays in the app by design (ADR-0001) |
| 7 | Three parallel build scripts, no canonical one | Infrastructure | **16** | ✅ Done — Nuitka and PyInstaller scripts removed; `build_cxfreeze.bat` (via `build.bat`) is the sole packager |
...
- **Jobs module (#3, #6):** the `Job` state container moved out of `app.py` into a new `ryos/jobs.py`, alongside a pure `format_elapsed(start, now)` for the running-row time label, and a `JobRegistry` that owns job storage + id allocation (replacing the inline `self._jobs` dict and `self._next_job_id` counter across 15 call sites). The subprocess worker `_run_subprocess` has since moved to a standalone `ryos/runner.py` (`run_subprocess`) — UI-free, communicating only via the output queue, and now tested end-to-end with real subprocesses (success, nonzero exit, missing binary). `app.py`'s method is a thin delegator. The output-queue **protocol decoding** has also moved to `runner.decode_output_item` (a pure tuple→`OutputAction` translation), so `_drain_output_queue` is now a thin map from decoded actions to widget calls — and the protocol is tested from both ends (producer `run_subprocess`, consumer `decode_output_item`, plus a round-trip). What remains on `RYOSApp` is genuinely widget-coupled: `_new_job` tab/timer setup, `_finish_job` running-row teardown, `_handle_step_done` status/notify dispatch, and the drain loop's `after()` tick. Those need a runtime smoke-test to touch further.
- **Job lifecycle controller (#6):** the remaining lifecycle logic — job allocation + the parallel-job cap (`new_job`/`at_capacity`), pipeline stepping (`run_next_pipeline_step`), step completion + notify (`handle_step_done`), and the output-queue drain (`pump`) — is now extracted into a UI-free `ryos/job_controller.py` (`JobController`), reaching the window only through injected callbacks (`on_output`/`on_status`/`on_notify`/`on_started`/`on_finish`/`on_rename`/`launch`). `RYOSApp._drain_output_queue` is now a two-line tick; `_new_job`, `_handle_step_done`, and `_run_next_pipeline_step` are gone from the god class. Covered by 18 new `TestJobController` tests (no display). See `docs/adr/0001-decompose-job-lifecycle.md`.
- **Build scripts (#7) & dedup:** `build_cxfreeze.bat` is the canonical/supported build; the Nuitka and PyInstaller scripts have since been removed, leaving cx_Freeze as the only packager. Also deduped the Quick Run skip-dirs set — `app.py`'s index-builder now imports `quickrun._SKIP_DIRS` instead of keeping its own copy, so the search and autocomplete index paths can't drift.

**Test suite:** grew from a **red 45-passing baseline** (2 stale tests were failing) to **156 passing / 3 skipped**, now covering interpreter (`detect_interpreter`, `build_command`, `resolve_interpreter` incl. the RYOS.exe guard, `_script_tag`), DB CRUD/ordering/migrations + versioning + groups (rename propagation, orphan-on-delete, deep clone) + pipelines (steps join, cascade delete, deep clone, reorder) + reorder/move-between-groups + `mark_run_status` + export/import round-trip incl. pipelines & base_dir, `settings` load/save fallbacks + round-trip, `notifications._parse_version`, the full `quickrun` module, and `jobs` (`Job` defaults, `format_elapsed`, `JobRegistry`). The DB persistence layer — where bugs mean silent data loss — is now comprehensively protected.
...
2. ~~Extract the pure cores of `_quick_run_submit`~~ done — `parse_input` + `display_relpath` now in `quickrun`, tested. (`_handle_step_done` is genuine UI/notification dispatch; not worth extracting.)
3. ~~Continue peeling subsystems off `RYOSApp` (#6) — the job lifecycle is the natural next seam.~~ done — extracted into `job_controller.JobController` (allocation, capacity, pipeline stepping, completion, drain), tested. What remains on `RYOSApp` is widget construction, which stays by design.
...
A single class owns UI construction, drag-and-drop, tab management, group CRUD, subprocess orchestration, the quick-run subsystem, output routing, notifications, and update checks — ~100 methods over ~2,200 lines. High mechanical effort to split, which is why the priority lands mid-table despite high impact. **Resolved:** the job lifecycle now lives in the UI-free `job_controller.JobController`; what remains on `RYOSApp` is widget construction, kept in the app by design. See ADR-0001 and the Progress section.
...
`build_cxfreeze.bat` is the sole packager; `build.bat` delegates to it. (The Nuitka and PyInstaller scripts have been removed.)
