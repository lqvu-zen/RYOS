...
| `ryos/sections.py` | A group's sections (Favorites, Pipelines, Scripts, and Running above them): which records go where (`split`), the header and empty-section texts, and a per-group `CollapseState` for the session. A favourite appears in Favorites and in its own section. Shared by the Tk and Qt pages. | yes |
