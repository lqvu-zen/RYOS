...
| 2.8 Flip `__main__` | **done 2026-09-25** — `uv run ryos` and RYOS.exe start the Qt app (`qtui/main.py`). `RYOS_UI=tk` still starts Tk, as does a missing PySide6. Dependencies are now PySide6 alone (tkinterdnd2, pystray and pillow gone), and the build is 108 MB, with unused Qt libraries excluded. `tests/launch_smoke.py` starts the real entry point, from source or the exe, on a throwaway data folder, with registry writes off (`RYOS_NO_REGISTRY`), and checks the user's own folder is unchanged. CI now runs the Qt smoke and the launch smoke offscreen. Next: remove `ryos/ui/` once the Qt release has settled. |
...
| ~~Window grows a few pixels on each start~~ | — | **Fixed 2026-09-25.** Cause: moving a window onto a monitor with a different scale factor (here 100% and 125%) makes Windows send its own resize and move ~2 ms after the call returns. That is its suggested rectangle, scaled from the old monitor, and a frame's width off. It was intermittent in the smoke because it depended on which screen the new window was born on. Without a fix, three save/restore cycles went 542x648, 544x656, 546x664 (125%→100%) and 538x634, 537x628, 535x622 (100%→125%). For 0.5 s after each placement, `set_geometry_string` puts the requested geometry back if the window drifts by a frame's width or less (at most 24 px, at most three times). A real drag, or the app's own resize, moves further and is left alone. Detecting the scale change up front was tried first and missed some cases: a new window's screen is not known to Qt until Windows has shown it. Placement computes one final geometry, snap included, so a correction cannot undo a snap. `check_mixed_dpi_placement` runs three cycles each way, both settled and placed straight after show(), on any machine with two differently scaled screens, and says it skipped otherwise. Tk is unaffected: it is DPI-unaware, so Windows scales it. |
| ~~"All" tab~~ | — | **Done 2026-09-25.** Qt has an All tab, last, showing every group under its own header, with ungrouped items under "Other" (no header when there are no named groups) and the "No scripts yet" message when empty. It is not a group: it has no key, so dropping on it, right-clicking it and dragging tabs past it do nothing to any group. Its blocks work like the group tabs: drops, moves, collapse (shared with the group's own tab) and select mode across every group. The block rule and wording are in `sections.py`, and Tk uses them. Found on the way: **"Remember last group" never worked in Tk**. The setting was read at start but nothing ever wrote it. Both apps now save the group on screen when quitting, and open on it (`grouping.initial_group` / `remember_group`). The Qt Add button now reads "+ Script", as in Tk. |
| ~~Running section per group~~ | — | **Decided 2026-09-25: keep Qt's single running list** under the tabs. It is visible whichever tab is open, and the tray already jumps to a job's output. |
| ~~Run with parameters~~ | — | **Done 2026-09-25.** The card's preset drop-down chooses what Run passes. A script that asks each run gets the prompt, and cancelling it runs nothing. ▶+ runs with what was typed and keeps it as the parameters and a preset, as in Tk. The rules are in `scriptform.py` and Tk uses them. Found on the way: the Qt temp-parameter prompt pre-filled the saved parameters, and the answer is appended to them, so they would have been passed twice. It now shows them as a label, as Tk does. |
| ~~+ Pipeline~~ | — | **Done 2026-09-25.** A + Pipeline button names a pipeline in the group on screen and opens the editor. On the All tab it asks you to pick a group first, as Tk does. Wording is shared in `pipelinesteps.py`. |
| ~~Output panel: search, filter, tab menu, collapse~~ | — | **Done 2026-09-25.** Each Qt output tab now keeps its lines with their tags and colours them (stderr was not red in Qt: the tag was dropped). Errors only hides and restores lines, find counts and steps (Enter / Shift+Enter, Esc clears), and each tab keeps its own search and filter. The tab menu has Copy / Save / Close. Clear and Close All (which keeps running tabs) work as in Tk. The panel starts collapsed, opens for a new job when auto_open_output is set, and opens when the tray jumps to a job. Find and the filter sit on a second row: one row was wider than the window's default 540 px, and a layout's minimum width becomes the window's. Wording and rules are in `outputpanel.py`, and Tk uses them. |
| ~~Drop files onto the window~~ | — | **Done 2026-09-25.** Files dropped on the Qt window become scripts in the group on screen (ungrouped from All), with detected interpreters. Folders are ignored, files outside the group's base folder are skipped with a warning, and with no groups it asks for one first. Card drags are left to the card lists and tabs. The rules are in `scriptform.py`, shared with Tk. Found on the way: drops arrive with forward slashes on Windows (QUrl, and tkdnd), so a dropped file was stored differently from the same file added any other way; paths are now normalised for both. Qt's native drops also retire `tkinterdnd2`. |
| ~~Card badges, group banner, hover preview~~ | — | **Done 2026-09-25.** Qt cards show the ask-each-run and scheduled badges (the pipeline's 🕒 too), and a group tab opens with its base-folder banner, which is clicked to set the folder. Compact cards have the hover preview when it is on, and a pipeline's step count opens its steps. Badges, preview rows and banner text are in `cardstyle.py` / `sections.py`, shared with Tk. Unlike Tk, the All tab shows no banner per group. |
