...
| Drop files onto the window | `app._setup_file_drop` (tkinterdnd2) | Found in the pre-flip audit. Dropping files on the Tk window adds them as scripts; Qt does nothing. |
| Card badges, group banner, hover preview | `ui/cards.py`, `_render_group_banner` | Found in the pre-flip audit. No scheduled or temp-param badge on Qt cards, no base-folder banner on a group page, no hover preview on compact cards. |
