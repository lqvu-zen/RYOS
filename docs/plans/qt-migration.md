...
| Script dialog | `ui/dialogs.py` ScriptDialog | The Qt dialog has name, path, parameters and interpreter only, and saves nowhere. Missing: group, relative path under a base folder, ask-for-parameter, launcher, working folder, environment variables, presets, Delete. Nothing in the Qt window adds a script yet, and the script card's ⚙ is not wired. |
