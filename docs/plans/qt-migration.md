...
| Output panel: search, filter, tab menu, collapse | `app` output search, `_show_tab_context_menu`, `_toggle_output` | Found in the pre-flip audit. The Qt output tabs have no search or filter, no Copy / Save / Close menu, and the panel cannot be collapsed. |
